...
class ServerThread(threading.Thread):
    ...
    def run(self):
        while(True):
            ...
            request = network.recv_msg_GS(self.server_socket).split(";")
            ...
            if code_request == INSCRIPTION:
                write_log_recv(self.server_socket, "inscription", params)
                if len(params) != 4:
                    ret = BAD_REQUEST_FORMAT
                else:
                    ret = inscription_treatments(params[0], params[1], params[2], params[3])
            # ========== VERIF MAIL ==========
            elif code_request == VERIFIE_MAIL:
                write_log_recv(self.server_socket, "verifie mail", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== LOGIN ==========
                ...
                write_log_recv(self.server_socket, "login", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== STAT DETAILLEES ==========
                ...
                write_log_recv(self.server_socket, "stat det", params)
                if len(params) != 3:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== STAT GLOABLE ==========
                ...
                write_log_recv(self.server_socket, "stat globale", params)
                if len(params) != 1:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== LOGOUT ==========
                ...
                write_log_recv(self.server_socket, "logout", params)
                if len(params) != 1:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== CREATE ROOM ==========
                ...
                write_log_recv(self.server_socket, "create room", params)
                if len(params) != 1:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== JOIN ROOM ==========
                ...
                write_log_recv(self.server_socket, "join room", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== REDIRECT TO LOBBY ==========
                ...
                write_log_recv(self.server_socket, "redirect lobby", params)
                if len(params) != 1:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== LEAVE ROOM ==========
                ...
                write_log_recv(self.server_socket, "leave room", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== GET CATEGORIE ==========
                ...
                write_log_recv(self.server_socket, "get cat", params)
                if len(params) != 0:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== CREATE PION ==========
                ...
                write_log_recv(self.server_socket, "create pion", params)
                if len(params) != 3:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== GET QUESTION ==========
                ...
                write_log_recv(self.server_socket, "get question", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== UPDATE PION ==========
                ...
                write_log_recv(self.server_socket, "update pion", params)
                if len(params) != 10:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== END GAME ==========
                ...
                write_log_recv(self.server_socket, "end game", params)
                if len(params) != 2:
                    ret = BAD_REQUEST_FORMAT
                else:
            ...
            # ========== END GAME ==========
                ...
                write_log_recv(self.server_socket, "maintenance", params)
                maintenance()
                ret = SUCCESS
            # ========== PAS RECONNU ==========
            else:
                write_log_recv(self.server_socket, "unknown request code", params)
                ret = UNKNOWN_REQUEST_CODE
            ret = ";".join(str(val) for val in ret)
            write_log_send(self.server_socket, ret)
            network.send_msg_GS(ret, self.server_socket)
        return
